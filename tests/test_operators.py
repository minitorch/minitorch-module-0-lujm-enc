...
def test_sigmoid(a: float) -> None:
    ...
    * It is strictly increasing.
    """
    s_a = sigmoid(a)
    # Only check strict monotonicity in the non-saturated region
    if not is_close(sigmoid(a), 1):
        assert 0.0 < s_a < 1.0, "Sigmoid output must be between 0 and 1 (inclusive)"
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    sum12 = sum(ls1)
    for i in ls2: sum12 += i
    assert_close(sum(ls1) + sum(ls2), sum12)
